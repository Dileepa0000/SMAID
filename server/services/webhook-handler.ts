...
import { payoutResolverService } from "./payout-resolver-service";
...
export class WebhookHandler {
  ...
  private static async handleIncomingMessage(
    ...
    try {
      ...
      // ---- SMAID: Payout Issue Keyword Detection ----
      // Detect payout-related keywords from inbound customer messages and
      // auto-create a payout_issue record for Admin Review Queue.
      try {
        if (channelId && content && message.type === "text") {
          await WebhookHandler.handleSmaidPayoutDetection(
            channelId,
            channel.length > 0 ? channel[0] : null,
            contact[0],
            content,
            message.from
          );
        ...
        console.error("[SMAID] Payout detection error (non-blocking):", smaidErr);
      }
...
- When escalating, be polite and tell the user you are transferring them to a human agent.`;
  ...
  /**
   * SMAID — Payout Issue Keyword Detector
   * Runs non-blocking after every inbound text message.
   * If a customer message contains payout-related keywords, a payout_issue
   * record is auto-created in the Admin Review Queue and a WhatsApp
   * acknowledgement is sent back to the customer.
   */
  private static async handleSmaidPayoutDetection(
    channelId: string,
    channelData: any,
    contactData: any,
    messageContent: string,
    ...
    const PAYOUT_KEYWORDS = [
      "payout", "payout issue", "payment failed", "payment not received",
      "bank details", "bank account", "update bank", "change bank",
      "money not received", "withdrawal failed", "transfer failed",
      "refund", "refund issue", "settlement", "commission not paid",
    ];

    const msgLower = messageContent.toLowerCase().trim();
    const triggered = PAYOUT_KEYWORDS.some((kw) => msgLower.includes(kw));

    if (!triggered) return;

    console.log(`[SMAID] 🔔 Payout keyword detected from ${senderPhone}: "${messageContent.substring(0, 80)}"`);

    // Auto-create payout issue in DB
    const issue = await payoutResolverService.createPayoutIssue({
      customerPhone: senderPhone,
      customerName: contactData?.name || senderPhone,
      channelId,
      issueCategory: "payout",
      failureReason: `[Auto-detected via WhatsApp Bot] Customer message: "${messageContent.substring(0, 500)}"`,
    } as any);

    console.log(`[SMAID] ✅ Payout issue created: ${issue.id} for ${senderPhone}`);

    // Send WhatsApp acknowledgement back to customer
    try {
      if (channelData) {
        const waApi = new WhatsAppApiService(channelData);
        const ackMessage = `✅ *Support Ticket Raised!*\n\nWe have received your payout query and created a support ticket (ID: \`${issue.id.substring(0, 8).toUpperCase()}\`).\n\nOur admin team will review and process your request within 24–48 hours. Thank you for your patience! 🙏`;
        await waApi.sendTextMessage(senderPhone, ackMessage);
        console.log(`[SMAID] 📨 Acknowledgement sent to ${senderPhone}`);
      }
    } catch (sendErr) {
      console.error("[SMAID] Failed to send WhatsApp acknowledgement (non-blocking):", sendErr);
    }
  }
