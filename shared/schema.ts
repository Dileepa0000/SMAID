...
export const payoutIssues = pgTable(
  "payout_issues",
  {
    id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
    ...
    issueCategory: text("issue_category").notNull().default("payout"),
  ...
  (table) => ({
    payoutStatusIdx: index("payout_issues_status_idx").on(table.status),
    payoutPhoneIdx: index("payout_issues_phone_idx").on(table.customerPhone),
  })
);

export const insertPayoutIssueSchema = createInsertSchema(payoutIssues).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
